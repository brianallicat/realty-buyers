...
export default function RecentSalesView() {
  ...
  const uniqueZips = useMemo(() => [...new Set(recentSales.map((s) => s.zip))].sort(), []);
  ...
  const filtered = useMemo(() => {
    ...
    return results;
  }, [searchTerm, dateRange, financingFilter, propertyFilter, zipFilter, sortField, sortDir]);
  ...
  const financingDist = useMemo(() => {
    ...
    filtered.forEach((s) => {
      const label = financingLabels[s.financingType];
      counts[label] = (counts[label] || 0) + 1;
    });
    return Object.entries(counts).map(([name, value]) => ({ name, value }));
  }, [filtered]);

  const PIE_COLORS = ['#22c55e', '#3b82f6', '#f59e0b', '#8b5cf6', '#6b7280'];
  ...
  return (
    <div className="p-6 bg-[#0a1628] min-h-screen text-white">
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold">Recent Home Sales</h1>
          <p className="text-sm text-gray-400">Recorded sales — Maricopa County public records</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-[#0d1f3c] border border-white/10 rounded-lg p-4">
          <p className="text-xs uppercase tracking-wider text-gray-400 mb-1">Total Sales</p>
          <p className="text-2xl font-bold text-white">{filtered.length}</p>
          <p className="text-xs text-gray-500">{totalRecentSales} all time</p>
        </div>
        <div className="bg-[#0d1f3c] border border-white/10 rounded-lg p-4">
          <p className="text-xs uppercase tracking-wider text-gray-400 mb-1">Total Volume</p>
          <p className="text-2xl font-bold text-green-400">{formatCurrency(filteredVolume)}</p>
          <p className="text-xs text-gray-500">{formatCurrency(totalSalesVolume)} all time</p>
        </div>
        <div className="bg-[#0d1f3c] border border-white/10 rounded-lg p-4">
          ...
          <p className="text-xs text-gray-500">{formatCurrency(avgSalePrice)} all time</p>
        </div>
        <div className="bg-[#0d1f3c] border border-white/10 rounded-lg p-4">
          <p className="text-xs uppercase tracking-wider text-gray-400 mb-1">Cash Sales</p>
          <p className="text-2xl font-bold text-yellow-400">{cashPct}%</p>
      ...
      <div className="bg-[#0d1f3c] border border-white/10 rounded-lg p-4 mb-6 space-y-3">
        <div className="flex gap-3 items-center flex-wrap">
          ...
          >
            ...
            <option value="zip-asc">Zip Code A-Z</option>
          </select>
        </div>
      ...
      {showMap && (
        <div className="bg-[#0d1f3c] border border-white/10 rounded-lg overflow-hidden mb-6" style={{ height: 400 }}>
          <MapContainer center={[33.45, -111.94]} zoom={10} style={{ height: '100%', width: '100%' }} scrollWheelZoom={true}>
            ...
            />
            {filtered.map((sale) => (
              <CircleMarker key={sale.id} center={[sale.lat, sale.lng]} radius={8} fillColor={financingColors[sale.financingType]} fillOpacity={0.8} stroke={true} color="#fff" weight={1}>
                <Popup>
          ...
          <div className="flex items-center gap-4 px-4 py-2 border-t border-white/10 text-xs text-gray-400">
            <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full inline-block" style={{ backgroundColor: financingColors.cash }}></span> Cash</span>
            <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full inline-block" style={{ backgroundColor: financingColors.conventional }}></span> Conventional</span>
            <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full inline-block" style={{ backgroundColor: financingColors.fha }}></span> FHA</span>
            <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full inline-block" style={{ backgroundColor: financingColors.va }}></span> VA</span>
            <span className="ml-auto">{filtered.length} sales</span>
          </div>
        </div>
      )}
      ...
      <div className="bg-[#0d1f3c] border border-white/10 rounded-lg overflow-hidden mb-6">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {paged.length === 0 ? (
                <tr><td colSpan={10} className="px-4 py-12 text-center text-gray-500">No sales found matching your criteria.</td></tr>
              ) : (
                paged.map((sale) => (
                  <Fragment key={sale.id}>
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-green-400 font-medium whitespace-nowrap">{formatCurrency(sale.salePrice)}</td>
                      ...
                      <td className="px-4 py-3 text-gray-300 whitespace-nowrap">{homeTypeLabels[sale.propertyType]}</td>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-[#0d1f3c] border border-white/10 rounded-lg p-4">
          <h3 className="text-sm font-semibold text-white mb-4">Sales by Month</h3>
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={salesByMonth}>
              ...
              <Bar dataKey="count" fill="#3b82f6" radius={[4, 4, 0, 0]} name="Sales" />
            </BarChart>
          </ResponsiveContainer>
        </div>
